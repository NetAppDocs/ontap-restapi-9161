...
| 65539434
| Cannot perform key migration on the SVM while the keystore is being initialized. Wait until the keystore is in the active state, and rerun the key migration operation.

| 65539435
| Cannot perform key migration on the SVM while the keystore is being disabled.
|===

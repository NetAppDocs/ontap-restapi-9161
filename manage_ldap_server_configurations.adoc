...
=== Retrieving all of the LDAP configurations that have the _restrict_discovery_to_site_ set to _true_
...
# The call:
curl -X GET "https://<mgmt-ip>/api/name-services/ldap?restrict_discovery_to_site=true" -H "accept: application/hal+json"
...
=== Creating an LDAP configuration with Active Directory domain specified and _restrict_discovery_to_site_ set to _true_
